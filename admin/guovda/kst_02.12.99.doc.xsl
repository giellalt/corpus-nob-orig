...
<!-- Format query results for display --><xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:i18n="http://apache.org/cocoon/i18n/2.1" version="1.0">
    ...
    -->
    <xsl:variable name="filename" select="'kst_02.12.99.doc'"/>
    <xsl:variable name="text_encoding" select="''"/>
    <xsl:variable name="title" select="'MØTEBOK'"/>
    <!--
    ...
    <xsl:variable name="author1_born" select="'1968'"/>
    <!--
    ...
    <xsl:variable name="wordcount" select="'14424'"/>
...
<xsl:element name="p">
            ...
            <xsl:call-template name="globalTextReplace">
               <xsl:with-param name="inputString" select="$text"/>
               <xsl:with-param name="target" select="'O˜˜a/„/'"/>
               <xsl:with-param name="replacement" select="'Ođđa/č/'"/>
                <xsl:with-param name="continue" select="0"/>
            </xsl:call-template>
</xsl:element>
</xsl:template>
