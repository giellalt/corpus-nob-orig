...
<!-- Format query results for display --><xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:i18n="http://apache.org/cocoon/i18n/2.1" version="1.0">
    ...
    <xsl:variable name="author1_ln" select="'Johnsen'"/>
    <!-- m, f or unknown -->
    ...
    <xsl:variable name="wordcount" select="'16755'"/>
    ...
    <!-- Automatically filled in by CorpusTools -->
    <xsl:variable name="template_version" select="' 1.18 '"/>
    <!-- Automatically filled in by CorpusTools -->
    ...
    <xsl:variable name="parallels">
        ...
        <parallel_text xml:lang="sme" location="samisk_strategiplan_samisk.doc"/>
...
<xsl:template match="p">
    ...
    <xsl:element name="p">
        ...
        <xsl:call-template name="globalTextReplace">
           <xsl:with-param name="inputString" select="$text"/>
           <xsl:with-param name="target" select="'dr.-philos/'"/>
           <xsl:with-param name="replacement" select="'dr.philos/'"/>
           <xsl:with-param name="continue" select="0"/>
        </xsl:call-template>
    </xsl:element>
</xsl:template>
