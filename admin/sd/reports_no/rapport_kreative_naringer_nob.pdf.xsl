...
<!-- Format query results for display --><xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:i18n="http://apache.org/cocoon/i18n/2.1" version="1.0">
    ...
    <xsl:variable name="parallels">
        ...
        <parallel_text xml:lang="sma" location="rapport_kreative_naringer_sma.pdf"/>
        <parallel_text xml:lang="sme" location="rapport_kreative_naringer_sme.pdf"/>
    ...
    <!--
        ...
        must be defined.
    -->
    <xsl:variable name="inner_right_margin" select="'12=5'"/>
    <xsl:variable name="inner_left_margin" select="'12=5'"/>
    <xsl:variable name="inner_top_margin" select="'12=10'"/>
    <xsl:variable name="inner_bottom_margin" select="'12=75'"/>
...
<xsl:template match="p[parent::body][not(./em | ./span)][text()]">
        ...
        <xsl:element name="p">
            ...
            <xsl:call-template name="globalTextReplace">
                <xsl:with-param name="inputString" select="$text"/>
                <xsl:with-param name="target" select="'-s/-v/-t/-d/-l/-e/-m/-p/-b/-n/-e/-i/-j/-a/'"/>
                <xsl:with-param name="replacement" select="'s/v/t/d/l/e/m/p/b/n/e/i/j/a/'"/>
                <xsl:with-param name="continue" select="0"/>
            </xsl:call-template>
        </xsl:element>
    </xsl:template>
