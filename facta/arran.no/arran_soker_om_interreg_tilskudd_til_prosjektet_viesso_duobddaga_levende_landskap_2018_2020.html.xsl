...
<!-- Format query results for display --><xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:i18n="http://apache.org/cocoon/i18n/2.1" version="1.0">
    ...
    -->
    <xsl:variable name="filename" select="'http://arran.no/aktuelt/?Article=192'"/>
    <xsl:variable name="text_encoding" select="''"/>
    <xsl:variable name="title" select="'ÁRRAN SØKER OM INTERREG-TILSKUDD TIL PROSJEKTET VIESSO DUOBDDÁGA/LEVENDE LANDSKAP 2018–2020'"/>
    <!--
    ...
    <xsl:variable name="parallels">
        ...
        <parallel_text xml:lang="smj" location="arran_ahtsa_interreg_ruhtadimdoarjjagav_prosjaektaj_viesso_duobddaga_levende_landskap_2018_2020.html"/>
    ...
    <xsl:template match="p[parent::body][not(./em | ./span)][text()]">
        ...
        <xsl:element name="p">
            ...
            <xsl:call-template name="globalTextReplace">
                ...
                <xsl:with-param name="target" select="'Ønsker du å motta nyhetsbrev fra arran?/Registrer inn din epostadresse under. Du kan også kontakte oss pr epost: poassta@arran.no eller telefon +47 75 77 51 00/Din epost/'"/>
