...
<!-- Format query results for display --><xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:i18n="http://apache.org/cocoon/i18n/2.1" version="1.0">
    ...
    <language xml:lang="sma"/><language xml:lang="nob"/></xsl:variable>
    ...
    <xsl:variable name="parallels">
        ...
        <parallel_text xml:lang="sma" location="index.php_egavpi-1_girji-231_giella1-sam.html"/>
...
<xsl:template match="p[parent::body][not(./em | ./span)][text()][contains(text(),'---')][text()]">
        <xsl:variable name="text" select="current()"/>
        <xsl:variable name="type" select="@type"/>
        <xsl:variable name="lang" select="@xml:lang"/>
	<xsl:variable name="sma_part" select="substring-before($text,'---')"/>
	<xsl:variable name="nob_part" select="substring-after($text,'---')"/>
