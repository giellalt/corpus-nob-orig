...
<!-- Format query results for display --><xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:i18n="http://apache.org/cocoon/i18n/2.1" version="1.0">
    ...
    -->
    <xsl:variable name="filename" select="'http://www.gierdu.no/pdf/JorunnSpordBorgen.pdf'"/>
    <xsl:variable name="text_encoding" select="''"/>
    <xsl:variable name="title" select="'Sámi dáidda, fápmu ja ávkkálaš girjáivuohta'"/>
    <!--
        Information of the first author: first name, last name,
        date or year of birth, nationality.
    -->
    <xsl:variable name="author1_fn" select="'Jorunn Spord'"/>
    <xsl:variable name="author1_ln" select="'Borgen'"/>
    <!-- m, f or unknown -->
    ...
    <xsl:variable name="translator_fn" select="'Brita'"/>
    <xsl:variable name="translator_ln" select="'Kåven'"/>
    ...
    <xsl:variable name="parallels">
        ...
        <parallel_text xml:lang="sme" location="jorunn_spord_borgen.pdf"/>
    ...
    <!--
        ...
        1, 6-10, 15, 20, 25-30
    -->
    <xsl:variable name="skip_pages" select="'10'"/>
    ...
    <xsl:variable name="bottom_margin" select="'1=75, 2;3;4;5;6;7;8;9=50'"/>
