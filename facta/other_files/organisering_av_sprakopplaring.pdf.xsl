...
<!-- Format query results for display --><xsl:stylesheet xmlns:i18n="http://apache.org/cocoon/i18n/2.1" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:variable name="filename" select="'Organisering_av_språkopplæring.pdf'"/>
  <xsl:variable name="title" select="'GIELLAOAHPAHUSA ORGANISEREN SÁMI OAHPPOPLÁNAID VUOĐUL '"/>
  ...
  <xsl:variable name="place" select="'http://www.skolenettet.no/nyUpload/Samisk læringsnett/Dokumenter/Organisering av språkopplæring.pdf'"/>
...
<!-- If the document has parallel texts, uncomment the right languages
    ...
    <xsl:variable name="parallels">
        <parallel_text xml:lang="sme" location="organisering_av_sprakopplaring"/>
    </xsl:variable>
    ...
    <!--
        ...
        1, 6-10, 15, 20, 25-30
    -->
    <xsl:variable name="skip_pages" select="'1-2, 21-52 '"/>
...
<xsl:element name="p">
            ...
            <xsl:call-template name="globalTextReplace">
               ...
               <xsl:with-param name="target" select="'ﬂ/'"/>
               <xsl:with-param name="replacement" select="'fl/'"/>
