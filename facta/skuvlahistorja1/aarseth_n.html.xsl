...
<!-- Format query results for display --><xsl:stylesheet xmlns:i18n="http://apache.org/cocoon/i18n/2.1" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:variable name="filename" select="'aarseth_n.htm'"/>
  <xsl:variable name="title" select="'Internatet som sosialt bygdesentrum'"/>
  <xsl:variable name="author1_fn" select="'Bjørn'"/>
  <xsl:variable name="author1_ln" select="'Aarseth'"/>
  <xsl:variable name="author1_gender" select="'m'"/>
  <xsl:variable name="author1_born" select="'1926'"/>
  ...
  <xsl:variable name="place" select="'http://girji.info/skolehist/aarseth-n.htm'"/>
  ...
  <xsl:variable name="wordcount" select="'4103'"/>
...
<xsl:variable name="parallels">
	<parallel_text location="aarseth_s.html" xml:lang="sme"/>
</xsl:variable>
