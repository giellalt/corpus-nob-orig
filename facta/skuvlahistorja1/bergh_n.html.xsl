...
<!-- Format query results for display --><xsl:stylesheet xmlns:i18n="http://apache.org/cocoon/i18n/2.1" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:variable name="filename" select="'bergh_n.htm'"/>
  <xsl:variable name="title" select="'Mine år på Skoganvarre skole'"/>
  <xsl:variable name="author1_fn" select="'Richard'"/>
  <xsl:variable name="author1_ln" select="'Bergh'"/>
  ...
  <xsl:variable name="place" select="'http://girji.info/skolehist/bergh-n.htm'"/>
  ...
  <xsl:variable name="wordcount" select="'3960'"/>
...
<xsl:variable name="parallels">
	<parallel_text location="bergh_s.html" xml:lang="sme"/>
</xsl:variable>
